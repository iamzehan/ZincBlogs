...
export default function Page() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!username || !email || !firstName || !lastName || !password || !confirmPassword || password !== confirmPassword) {
  ...
  return (
    <Background>
      <div className="w-full max-w-md p-8 rounded-2xl bg-zinc-900/60 backdrop-blur border
       border-zinc-800 shadow-xl shadow-zinc-900/40">
        ...
        {submitted ? (
          ...
          <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
            <Input key="username" label="Username" name="username" placeholder="johndoe123"/>
            <Input key="email" label="Email" name="email" type="email" placeholder="john.doe@example.com"/>
            
            <div className="flex gap-4">
              <Input key="firstname" label="First Name" name="firstName" placeholder="John" />
              <Input key="lastname" label="Last Name" name="lastName" placeholder="Doe" />
            </div>
